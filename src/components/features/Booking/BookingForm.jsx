// src/components/features/Booking/BookingForm.jsx
...
function BookingForm({ 
  ...
  const handleSubmit = async (event) => {
    ...
    if (!selectedDate) {
      setSubmissionStatus({ message: 'Por favor, selecciona una fecha del calendario.', type: 'error' }); return;
    }
    if (!selectedSlot) {
      setSubmissionStatus({ message: 'Por favor, selecciona una franja horaria.', type: 'error' }); return;
    }
    ...
    const bookingData = {
      // Nombres de clave EXACTOS que espera la Cloud Function
    ...
    try {
      ...
      setSubmissionStatus({ message: error.message || 'Ocurrió un error al enviar la reserva.', type: 'error' });
    } finally {
      setIsSubmitting(false);
    }
  };

  // Clases para los botones de franja
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-200 space-y-6">
      ...
      {submissionStatus.message && (
        <p role="alert" aria-live="assertive" className={`text-sm text-center mt-3 ${submissionStatus.type === 'success' ? 'text-green-700' : 'text-red-600'}`}>
          {submissionStatus.message}
        </p>
      )}
    </form>
